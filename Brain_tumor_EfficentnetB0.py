...
#Load pre-train weight learned from imagenet dataset, include_top exclude the 'top' of network to customize with own layer for specific task
effnet = EfficientNetB0(weights='imagenet', include_top=False, input_shape=(image_size, image_size, 3))

# Build the model
model = effnet.output #take the output of the model, output is 2D feature map
#Performs global average pooling reducing each feature map to a single number(average value)
model = tf.keras.layers.GlobalAveragePooling2D()(model)
#Prevent overfitting, 0,5 mean half of unit (pixel value) will be random drop each training step
model = tf.keras.layers.Dropout(rate=0.5)(model)
#Add fully connect layer with number of unit equal to number of label, softmax ensure number of output = 1
model = tf.keras.layers.Dense(len(labels), activation='softmax')(model)
#Define complete model by specifying input: input layer of efficenetb0 and output: output of dense layer
model = tf.keras.models.Model(inputs=effnet.input, outputs=model)

# Compile the model
#Loss function measures the difference between true labels and the predicted probabilities. Using Adam optimizer algorithm.
# Accuracy use as the metric evaluate model perfomance, measures the percentage of correct predictions made by the model
